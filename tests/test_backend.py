...
def test_enrichment_creates_single_word_derived_clips(monkeypatch):
    source_id = db.create_source("demo", "youtube", "/tmp/demo.wav", "complete")
    db.set_source_voice_profile(source_id, "source-voice-7")
    monkeypatch.setattr(
        enrichment,
        "synthesize_word",
        lambda word, voice_id: AudioSegment.silent(duration=100, frame_rate=16000),
    )
    result = enrichment.enrich_missing_words(source_id, ["signal"], target_variants=2)
    assert len(result["created"]) == 2
    clips = db.get_clips_for_word("signal")
    assert {clip["provenance"] for clip in clips} == {"qwen_derived"}
    assert {clip["voice_profile_id"] for clip in clips} == {"source-voice-7"}


def test_enrichment_requires_source_voice_profile():
    source_id = db.create_source("demo", "youtube", "/tmp/demo.wav", "complete")
    with pytest.raises(ValueError, match="voice profile"):
        enrichment.enrich_missing_words(source_id, ["signal"])


def test_qwen_word_synthesis_rejects_sentences():
    with pytest.raises(QwenCloudError, match="exactly one word"):
        synthesize_word("not a word", "voice-1")
...
def test_provider_status_describes_qwen_as_enrichment(monkeypatch):
